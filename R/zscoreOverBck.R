...
#'
#' @param ratios A numeric vector containing the transformed, background
#'  corrected and smoothed ratios in each window.
#' @param backgroundPercentage numeric(1). Low percentile for background
#' distribution.
...
zscoreOverBck <- function(ratios, backgroundPercentage = 0.25)
{
    ...
    pop.sd <- sd(bg.ratios) *
        sqrt(length(bg.ratios) - 1)/sqrt(length(bg.ratios))
